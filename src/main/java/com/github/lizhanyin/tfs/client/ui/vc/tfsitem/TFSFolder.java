...
import com.github.lizhanyin.tfs.runtime.IActionFilter;
...
public class TFSFolder extends TFSItem implements IActionFilter {
    ...
    private void computeChildren(final boolean showDeletedItems) {
        ...
        for (TFSItem implicitChild : implicitChildren) {
            addChild(implicitChild);
        }
    }
    ...
    public Set<TFSItem> getFolderChildren(final boolean includeDeletedItems) {
        ...
        folderChildren.removeIf(tfsItem -> !(tfsItem instanceof TFSFolder));
    ...
    public boolean hasFolderChildren() {
        return !getFolderChildren().isEmpty();
    }
    ...
    public boolean testAttribute(final Object target, final String name, final String value) {
        if (target instanceof TFSFolder folder) {
            ...
            {
                switch (value) {
                    case "Unmapped" -> {//$NON-NLS-1$
                        return !isMapped && exactFolder == null;
                    }
                    case "ExactMapping" -> {//$NON-NLS-1$
                        return exactFolder != null && !exactFolder.isCloaked();
                    }
                    case "MappedByParent" -> {//$NON-NLS-1$
                        return isMapped && exactFolder == null;
                    }
                    case "Cloaked" -> {//$NON-NLS-1$
                        return exactFolder != null && exactFolder.isCloaked();
                    }
                }
            }
