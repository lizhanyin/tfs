...
package com.github.lizhanyin.tfs.client.server.cache.buildstatus;
...
import com.github.lizhanyin.tfs.client.prefs.PreferenceConstants;
...
public class BuildStatusManager {
    ...
    private void loadConfiguration() {
        final PropertiesComponent prefs = PropertiesComponent.getInstance();

        final String refreshIntervalStr = prefs.getValue(PreferenceConstants.BUILD_STATUS_REFRESH_INTERVAL);

        if (refreshIntervalStr != null) {
            try {
                final int refreshIntervalMillis = Integer.parseInt(refreshIntervalStr);

                if (refreshIntervalMillis > 0) {
                    refreshInterval = refreshIntervalMillis;
                }
            } catch (final NumberFormatException e) {
                log.warn("Invalid refresh interval preference value: " + refreshIntervalStr, e); //$NON-NLS-1$
            }
        }
    }
    ...
    private void loadExtensionListeners() {
        /*
         * Eclipse extension point mechanism is not available in IDEA.
         * Listeners should be registered programmatically via addListener().
         */
    }
    ...
    private void refreshInternal() {
        ...
        try {
            ...
            final List<Integer> queryIdList;
            ...
            synchronized (watchedBuildLock) {
                 ...
                 */
                final List<Integer> externallyRemovedList = new ArrayList<>(watchedBuilds.keySet());
                ...
                queryIdList = new ArrayList<>(statusCache.getBuilds());
                ...
                for (final Integer removedId : externallyRemovedList) {
                    log.debug(
            ...
            if (log.isDebugEnabled()) {
                if (queryIdList.isEmpty()) {
                    ...
                    final StringBuilder buildIdDebugList = new StringBuilder();

                    for (int i = 0; i < queryIdList.size(); i++) {
                        buildIdDebugList.append((i > 0) ? ", " : "").append(queryIdList.get(i)); //$NON-NLS-1$ //$NON-NLS-2$
                    }
    ...
    private class BuildStatusManagerRefreshWorker implements Runnable {
        ...
        private void startRefreshJob() {
            final String jobName = Messages.getString("BuildStatusManager.BuildStatusRefreshJobName"); //$NON-NLS-1$
            ProgressManager.getInstance().run(new Task.Backgroundable(null, jobName, false) {
                @Override
