...
public abstract class RepositoryMap {
     ...
     */
    private final Map<Object, TFSRepository> toRepository = new HashMap<>();

    /**
     * Maps from {@link TFSRepository} to {@link Set} of mapped objects. Guarded
     * by {@link #lock}. Never <code>null</code>.
     */
    private final Map<TFSRepository, Object> fromRepository = new HashMap<>();
    ...
    public final TFSRepository[] getRepositories() {
        synchronized (lock) {
            return (TFSRepository[]) fromRepository.keySet().toArray(new TFSRepository[0]);
        }
    }
    ...
    protected final void addMappingsInternal(final TFSRepository repository, final Object[] mappedObjects) {
        ...
        synchronized (lock) {
            Set<Object> set = (Set<Object>) fromRepository.get(repository);
            if (set == null) {
                set = new HashSet<>();
            ...
            for (Object mappedObject : mappedObjects) {
                ...
                toRepository.put(mappedObject, repository);
            }
        }
    }
    ...
    protected final Object[] getMappedObjectsInternal(final TFSRepository repository) {
        ...
        synchronized (lock) {
            final Set<Object> set = (Set<Object>) fromRepository.get(repository);
    ...
    protected final Object[] getAllMappedObjectsInternal() {
        final Set<Set> allMappedObjects = new HashSet<>();

        synchronized (lock) {
            for (Object o : fromRepository.values()) {
                final Set repositorySet = (Set) o;
