...
package com.github.lizhanyin.tfs.client.framework.command.helpers;
...
public class CommandFinishedCallbackHelpers {
    ...
    public static String getMessageForStatus(final IStatus status) {
        if (!status.isMultiStatus()) {
            ...
            if (!status.getMessage().isEmpty() && status.getException() != null) {
        ...
        for (IStatus child : children) {
            message.append("\n"); //$NON-NLS-1$
            message.append(getMessageForStatus(child));
        }
