...
public class Status implements IStatus {
    ...
    /**
     * Plugin ID for IntelliJ IDEA plugin.
     */
    private static final String IDEA_PLUGIN_ID = "com.github.lizhanyin.tfs"; //$NON-NLS-1$
    ...
    /**
     * Extracts an identifier from the given class: returns the class name or
     * plugin ID, never returns <code>null</code>
     *
     * @param caller the relevant class to build unique identifier from
     * @return identifier extracted for the given class
     */
    private static String identifier(@Nullable Class<?> caller) {
        return Optional.ofNullable(caller)
                .map(Class::getName)
                .orElse(IDEA_PLUGIN_ID);
    }
     ...
     * @param pluginId the unique identifier of the relevant plug-in
     */
    protected void setPlugin(@NotNull String pluginId) {
        if (pluginId == null || pluginId.isEmpty()) {
            throw new IllegalArgumentException("pluginId must not be null or empty"); //$NON-NLS-1$
        }
    ...
    protected void setSeverity(int severity) {
        if (severity != OK && severity != ERROR && severity != WARNING && severity != INFO && severity != CANCEL) {
            throw new IllegalArgumentException("Invalid severity value: " + severity); //$NON-NLS-1$
        }
